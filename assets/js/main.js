!(function (e) {
    ...
    if (
        (e(window).on("load", function () {
            e(".preloader").fadeOut(), new WOW({ boxClass: "wow", animateClass: "animated", offset: 0, mobile: !1, live: !0 }).init();
        }),
                ...
                if (a.length) {
                    ...
                    if (n) {
                        var i = new PANOLENS.ImagePanorama(s),
                            o = new PANOLENS.Viewer({ container: n, autoRotate: !0, autoRotateSpeed: 0.3, controlBar: !1, enableZoom: !0 });
                        n.addEventListener(
                            "wheel",
                            function (e) {
                                e.preventDefault();
    ...
    e(l).on("submit", function (e) {
        ...
        e(document).ready(function () {
            e(".popular-popup-image").magnificPopup({ type: "image", gallery: { enabled: !0 } }),
    ...
    (e.fn.inlineSvg = function () {
        ...
        e(document).click(function (t) {
            e(t.target).closest(".advance-search-wrapper, .advance-search-active, .select-group-wrapper").length ||
                (e(".advance-search-wrapper").removeClass("open").hide(), e(".advance-search-active").removeClass("active"), e(".advance-search-active i").removeClass("fa-times").addClass("fa-sliders-up"));
        }),
        document.addEventListener("DOMContentLoaded", function () {
...
//       .then(() => {
//         this.reset();
//         document.getElementById('newsletter-note').textContent = "✅ Subscribed successfully!";
//         // Optional: store locally
//         const saved = JSON.parse(localStorage.getItem('newsletterList') || '[]');
//         saved.push({ email: data.get("email") });
//         localStorage.setItem('newsletterList', JSON.stringify(saved));
//       })
...
//       .then(() => {
//         this.reset();
//         document.getElementById('contact-note').textContent = "✅ Message sent!";
//         // Optional: store locally
//         const messages = JSON.parse(localStorage.getItem('contactList') || '[]');
//         messages.push({
//           name: data.get("name"),
//           email: data.get("email"),
//           message: data.get("message")
//         });
//         localStorage.setItem('contactList', JSON.stringify(messages));
//       })
...
//     const successNote = document.getElementById('newsletter-note');

//     // Replace this with your actual email list form URL
//     const formUrl = "https://yourlistprovider.com/subscribe"; // 🔁 Replace with your link
...
//       // Success Message
//       successNote.textContent = "✅ Subscribed successfully!";
//       this.reset();
...
//     } catch (err) {
//       successNote.textContent = "⚠️ Something went wrong!";
//     }
//   });
  ...
  document.getElementById('newsletter-form').addEventListener('submit', e => {
    ...
    /* 2️⃣  fire-and-forget POST to your real e-mail-list URL */
    fetch('https://YOUR-EMAIL-LIST-LINK', {      // 🔁  REPLACE with your provider link
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body:   new URLSearchParams({ email })
    });

    /* 3️⃣  show success, reset form */
    e.target.reset();
  ...
  document.getElementById('contact-form').addEventListener('submit', e => {
    ...
    const entry = {
      ...
      message: document.getElementById('date').value.trim(),
      message: document.getElementById('desired_time').value.trim(),
      message: document.getElementById('message').value.trim(),
      time:    Date.now()
    };

    /* 1️⃣  store locally */
    const msgs = JSON.parse(localStorage.getItem('contactList') || '[]');
    msgs.push(entry);
    localStorage.setItem('contactList', JSON.stringify(msgs));

    /* 2️⃣  optional fire-and-forget POST to Netlify Forms
  ...
  document.getElementById('contact-form').addEventListener('submit', function(e) {
    e.preventDefault();  // Prevents form submission to server
    ...
    document.getElementById('contact-note').textContent = '✅ Message sent successfully!';
